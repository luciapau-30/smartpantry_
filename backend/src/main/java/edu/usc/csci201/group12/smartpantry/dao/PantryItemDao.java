...
public class PantryItemDao {
    ...
    /**
     * T3 background-thread query: joins PANTRY_ITEMS with INGREDIENTS so callers
     * can build a human-readable alert payload (e.g. "Tomatoes expire in 2 days")
     * without per-row lookup. Returns rows whose expiration_date falls within
     * {@code withinDays} of the current date (inclusive of today).
     */
